...
FROM mahasiswa
WHERE EXISTS
(SELECT * FROM nilai WHERE nilai.nim = mahasiswa.nim);

-- Soal 4
SELECT AVG(nilai_angka)
FROM
(SELECT nilai_angka FROM nilai WHERE kode_mk IN ('MK01', 'MK02')
)AS sementara;
...
FROM mahasiswa
JOIN nilai ON mahasiswa.nim = nilai.nim
JOIN mata_kuliah ON nilai.kode_mk = mata_kuliah.kode_mk;
